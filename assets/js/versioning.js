...
function hide_all() {
	var to_hide = document.querySelectorAll("section.v:not(.hide)")
	for(var i = 0; i < to_hide.length; i++){
		to_hide[i].classList.add("hide");
	}
}
...
function set_v_tags(ds) {
	document.querySelectorAll(".v-tag").forEach(function(element) {
...
function v_filter_page(v_dict) {
	...
	var vp;
	if (v) {
		vp = v.replace(/\./g,"-"); // use for classes
	} else {
		vp = null;
	}
	if (!vp) {
		...
		set_v_menu(v_default.v);
		set_v_tags(v_default.v);
		return
	}
	hide_all();
	set_v_menu(v);
	...
	document.querySelectorAll("section.v."+vp).forEach(function(element) {
...
function span_populate(v_urlparams) {
	document.querySelectorAll("span.v").forEach(function(element) {
	    element.textContent = v_urlparams.v;
	});
	if (!v_urlparams.v) {
		v_urlparams.v = "vX"
	}
